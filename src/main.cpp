...
int main(int argc, char** argv) {
    ...
    if (usePreprocessing) std::cout << "COM PRE-PROCESSAMENTO ";
    std::cout << "===" << std::endl;
    ...
    if (usePreprocessing) {
        ImageSegmentation::runSegmentationWithPreprocessing(inputImage, "out_tarjan.png", 
    ...
    if (usePreprocessing) {
        std::cout << std::endl << "DICA: O arquivo 'out_tarjan.png' foi gerado com pré-processamento melhorado!" << std::endl;
        std::cout << "Para comparar, execute sem --preprocess para ver a diferença." << std::endl;
    } else {
        std::cout << std::endl << "DICA: Execute com --preprocess para usar pré-processamento melhorado no Tarjan:" << std::endl;
